...
class BinanceClient:
    ...
    def resolve_base_url(self) -> str:
        ...
        market_type = os.getenv("BINANCE_MARKET_TYPE", "futures").strip().lower()
    ...
    def status(self) -> dict:
        ...
        result["price"] = self.public_get("/fapi/v1/ticker/price" if self.is_futures() else "/api/v3/ticker/price", {"symbol": self.symbol})
    ...
    def auto_trade_enabled(self) -> bool:
        return os.getenv("AUTO_TRADE_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"}
...
class BotHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/tradingview/webhook":
            client = BinanceClient(symbol=str(body.get("symbol", os.getenv("TRADE_SYMBOL", "BTCUSDT"))))
            try:
                result = client.handle_webhook_signal(body)
                self.send_json({"ok": True, "result": result})
            except Exception as error:  # noqa: BLE001 - return setup/order error to local UI.
